...
unsigned char limitadorADC = 0, limitador_vumetro = 0;
...
void Vumetro_Func(unsigned char Vumetro_Entero, unsigned char Vumetro_Resto) {
    ...
    if (!limitador_vumetro) {
        set_CURSOR(0x40);
        msg2LCD("              ");
        limitador_vumetro = 60;
    }

    set_CURSOR(0x40);

    while (Vumetro_Entero > 0) {
        char2LCD(1);
        Vumetro_Entero--;
    }

    if ((Vumetro_Resto > 0) && (Vumetro_Resto <= 2))
        char2LCD(5);

    else if ((Vumetro_Resto > 2) && (Vumetro_Resto <= 4))
        char2LCD(4);

    else if ((Vumetro_Resto > 4) && (Vumetro_Resto <= 6))
        char2LCD(3);

    else if ((Vumetro_Resto > 6) && (Vumetro_Resto <= 8))
        char2LCD(2);
...
void AD_Converter(void){
    ...
    long unsigned int Calculo_Prom = 0;
    unsigned char contador_prom = 0;
    int ConversionAD = 0;
    unsigned char Unidad, Decena, Centena, UniMil, UniDiezMil;
    unsigned char Vumetro = 0, VumetroF = 0;
    ...
    char2LCD(0);
            ...
            ConversionAD = obtener_ADC10();


            if (contador_prom < 60) {
                Calculo_Prom += ConversionAD;
                contador_prom++;
            } else {
                Calculo_Prom /= 60;
                contador_prom = 0;
                UV_bits = Calculo_Prom;
                Calculo_Prom = 0;
            }
        ...
        UniMil = (UVI % 10000) / 1000 + 48;
        Centena = ((UVI % 10000) % 1000) /100 + 48;
        Decena = (((UVI % 10000) % 1000) % 100) / 10 + 48;
        Unidad = (((UVI % 10000) % 1000) % 100) % 10 + 48;
        ...
        Vumetro = UVI / 938;
        VumetroF = (UVI % 938 * 10) / 938;

        Vumetro_Func(Vumetro, VumetroF);
...
void informar(const char *dato, char longitud){
    ...
    char i=0;
    for(i=0;i<longitud;i++) transmitir(*(dato+i));
...
void tic_debounce(void){
    static unsigned char debounce_tic = 10;
     if (debounce_tic)debounce_tic--;
    else {
        debounce_tic = 10;
        EstadoDeLosBotones = debounce((PORTB^0xF0)&0xF0);
    }
}
...
void EEwrite(unsigned char addr,unsigned char n) {
